...
namespace Hma.Desktop.Wpf.Presentation.Features.Settings.ViewModels;
    ...
    ThemeService theme, IUiOperationGate operationGate, IToastService toastService) : WorkspaceBase(operationGate, toastService)
{
    ...
    private EditorState? _parametersBaseline;
    private EditorState? _companyBaseline;
    ...
    private EditorState ParametersFingerprint =>
        EditorState.Capture(VatRate, DocumentStorePath, DispatchOrderLastValue, FreightStatementLastValue);

    private EditorState CompanyFingerprint =>
        EditorState.Capture(CompanyId, CompanyName, CompanyAddress, CompanyPhone, CompanyTaxCode, CompanyBank, CompanyWebsite, CompanyEmail);

    private bool CanLoadSettingsBody =>
        user.IsManager || user.Can(ScreenKeys.Settings, PermissionAction.View);
    ...
    private async Task SaveCompany()
    {
        ...
        await RunAsync(async () =>
        {
            var saved = await company.SaveAsync(new SaveCompanyCommand(
                CompanyId, CompanyName, CompanyAddress, CompanyPhone, CompanyTaxCode,
                CompanyBank, CompanyWebsite, CompanyEmail));
            CompanyId = saved.Id;
            _companyBaseline = CompanyFingerprint;
